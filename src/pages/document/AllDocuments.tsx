...
const AllDocuments = () => {
  const { deleteDocument, setDocuments, documents } = useDataStore();
  ...
  const [isLoading, setIsLoading] = useState(documents.length === 0);
  ...
  const loadDocuments = async (): Promise<void> => {
    try {
      ...
      // If we have data, just notify error
      if (documents.length > 0) {
        toast.error(`Failed to refresh: ${errorMessage}`);
      } else {
        setError(errorMessage);
        // Try to handle local fallback if something went wrong with store (unlikely with zustand persist)
      }
    } finally {
      setIsLoading(false);
            ...
            {selectedIds.size > 0 ? (
              ...
              <div>
                <div className="flex items-center gap-2">
                  ...
                  {isRefreshing && (
                    <div className="animate-spin rounded-full h-4 w-4 border-2 border-indigo-600 border-t-transparent" title="Refreshing data..."></div>
                  )}
                </div>
