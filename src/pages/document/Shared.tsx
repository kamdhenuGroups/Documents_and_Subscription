...
const SharedDocuments = () => {
    ...
    const fetchShareHistoryFromGoogleSheets = useCallback(async () => {
        try {
            setLoading(true);
            setError(null);
    ...
    }, [setTitle, fetchShareHistoryFromGoogleSheets]);

    const filteredData = useMemo(() => {
        const lowerSearchTerm = searchTerm.toLowerCase();
        return shareHistory.filter(item =>
            (item.shareNo || '').toLowerCase().includes(lowerSearchTerm) ||
            (item.docName || '').toLowerCase().includes(lowerSearchTerm) ||
            (item.recipientName || '').toLowerCase().includes(lowerSearchTerm) ||
            (item.docSerial || '').toLowerCase().includes(lowerSearchTerm) ||
            (item.contactInfo || '').toLowerCase().includes(lowerSearchTerm)
        );
    }, [shareHistory, searchTerm]);
