...
const ShareModal: React.FC<ShareModalProps> = ({
    ...
    const { addShareHistory, addShareHistories, shareHistory, documents } = useDataStore();
    ...
    const handleShareWhatsApp = (): void => {
        ...
        // Log the WhatsApp sharing activity (fire and forget in background)
        if (isBatch && batchDocuments.length > 0) {
            Promise.all(batchDocuments.map(doc =>
                logSharingActivity({
            ...
            )).catch(err => console.error("Error logging batch whatsapp share", err));
            ...
            logSharingActivity({
                ...
                shareMethod: 'WhatsApp',
                number: whatsapp
            }).catch(err => console.error("Error logging whatsapp share", err));
        }
    ...
    const handleSubmit = async (e: React.FormEvent): Promise<void> => {
        ...
        // Handle WhatsApp immediately to prefer UI responsiveness
        ...
        // Add to share history (Optimized Bulk Update)
        if (emailSuccess || whatsappOpened || type === 'whatsapp') {
            const nextId = shareHistory.length + 1;
            const historyItems: any[] = [];

            if (isBatch && batchDocuments.length > 0) {
                // Clean SN generation for batch
                let currentSnIndex = nextId;
                ...
                batchDocuments.forEach((doc, index) => {
                    ...
                    if (type === 'email' || type === 'both') {
                        historyItems.push({
                            ...
                            shareNo: `SH-${String(currentSnIndex++).padStart(3, '0')}`,
                            sharedVia: 'Email',
                            contactInfo: email
                        });
                    }
            ...
            if (historyItems.length > 0) {
                if (addShareHistories) {
                    addShareHistories(historyItems);
                } else {
                    // Fallback if not available
                    historyItems.forEach(item => addShareHistory(item));
                }
            }
