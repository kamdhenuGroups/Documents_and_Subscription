...
const SubscriptionApproval = () => {
    ...
    const [nextIdPromise, setNextIdPromise] = useState<Promise<string> | null>(null);
    ...
    const refreshData = async () => {
        try {
            ...
            // Helper for Consistent SN Normalization (Scoped locally)
            const normalizeSN = (input: any): string => {
                const str = String(input || "").trim();
                const match = str.match(/(\d+)/);
                if (match) {
                    return `SN-${match[0].padStart(3, '0')}`;
                }
                return str;
            };

            // 1. Fetch Both Sheets Simultaneously (Parallel Fetching)
            ...
            const [subJson, appJson] = await Promise.all([
                fetchSheet("Subscription"),
                fetchSheet("Approval")
            ]);

            if (!subJson.success) throw new Error("Failed to fetch subscriptions");

            // 2. Transform Base Subscriptions
            const rawSubs = subJson.data || [];
            const baseSubs: SubscriptionItem[] = rawSubs
                .filter((row: any[]) => {
                    if (!Array.isArray(row) || row.length < 2) return false;
                    const snRaw = String(row[1] || "").toLowerCase();
                    const company = String(row[2] || "").toLowerCase();

                    // Strip clearly invalid header rows
                    if (snRaw.includes('subscription') || snRaw.includes('serial') || company.includes('company_name')) {
                        return false;
                    }
                    // Must have SN and (Company OR Subscription Name)
                    return row[1] && (row[2] || row[4]);
                })
                .map((row: any[], index: number) => {
                    const normalizedSn = normalizeSN(row[1]);

                    return {
                        id: `sub-${normalizedSn}-${index}`,
                        sn: normalizedSn,
                        requestedDate: row[0] || "",
                        ...
                        purpose: row[7] || "N/A",
                        status: "Pending",
                        actual2: (row[14] || "").toString().trim(),
                        startDate: "",
                        endDate: ""
                    };
                });

            // 3. Index Base Subs for O(1) Lookup
            const subMap = new Map<string, SubscriptionItem>();
            baseSubs.forEach(sub => subMap.set(sub.sn, sub));

            // 4. Merge Approval Logs Efficiently
            if (appJson.success && Array.isArray(appJson.data)) {
                // Skip header
                const approvalLogs = appJson.data.slice(1);

                approvalLogs.forEach((log: any[]) => {
                    const logSubNo = log[2];
                    if (!logSubNo) return;

                    const normalizedLogSn = normalizeSN(logSubNo);
                    const status = log[4];
                    const apNo = log[1];
                    const note = log[5];
                    const date = log[0];

                    if (subMap.has(normalizedLogSn)) {
                        const sub = subMap.get(normalizedLogSn)!;
                        sub.status = status;
                        sub.approvalNo = apNo;
                        sub.remarks = note;
                        sub.approvalDate = date;
                    } else {
                        // Add Orphan with corrected SN
                        baseSubs.push({
                            id: `sub-orphan-${normalizedLogSn}-${Math.random()}`,
                            sn: normalizedLogSn,
                            requestedDate: "",
                            ...
                            startDate: "",
                            endDate: "",
                            actual2: "orphaned" // Mark as not pending
                        });
                    }
                });
            }
    ...
    const handleActionClick = (sub: SubscriptionItem) => {
        ...
        setRemarks('');
        // Optimization: Prefetch the next Approval ID while the user is filling the form
        setNextIdPromise(fetchNextApprovalSN());
    };

    const handleCloseModal = () => {
        setSelectedSub(null);
        setRemarks('');
        setNextIdPromise(null);
    };
    ...
    const handleSave = async () => {
        ...
        try {
            // Generate unique Approval No (Use prefetched if available)
            let apNo: string;
            if (nextIdPromise) {
                try {
                    apNo = await nextIdPromise;
                } catch (e) {
                    console.warn("Prefetch failed, retrying...");
                    apNo = await fetchNextApprovalSN();
                }
            } else {
                apNo = await fetchNextApprovalSN();
            }
